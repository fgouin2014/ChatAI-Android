...
class BidirectionalBridge private constructor(private val context: Context) {
    ...
    private val keyring: com.chatai.KeyringManager by lazy {
        com.chatai.KeyringManager.getInstance(context)
    }
    ...
    init {
        ...
        try {
            ...
            ragService = RAGService(context, conversationDao, embedding)
    ...
    suspend fun processWithThinking(
        ...
        try {
            ...
            if (!ragEnabled) {
                ...
                // ⭐ Utiliser variables locales pour éviter problèmes de smart cast
                ...
                if (embedding == null) {
                    ...
                    try {
                        ...
                        val similarConversations = try {
                            rag.searchSimilarConversations(
                        ...
                        if (!similarConversations.isNullOrEmpty()) {
                            ...
                            ragContext = try {
                                rag.buildRAGContext(similarConversations) ?: ""
                            } catch (e: Exception) {
