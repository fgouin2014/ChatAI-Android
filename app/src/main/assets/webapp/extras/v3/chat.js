/**
 * Chat IA Mobile - Version Optimisée et Sécurisée
 * Améliorations: Performance, UX, Sécurité, Code Clean
 */
class SecureMobileAIChat {
    ...
    async initialize() {
        ...
        this.connectWebSocket();
            ...
            this.showSecureMessage('ai', 'Erreur lors du démarrage. Veuillez rafraîchir la page.');
        }
    }
    ...
    /**
     * Initialisation des éléments DOM avec validation
     */
    initializeElements() {
        const elements = {
    ...
    /**
     * Attachement des événements avec gestion d'erreurs
     */
    ...
    setupConfigFormListeners() {
        ...
        // Les événements des cartes hotword sont gérés dans renderHotwordModelsTable()
    ...
    renderConfigForms() {
        ...
        // Appliquer actions mappées aux modèles par nom si présent
     ...
     */
    handleTypingEnd() {
        // Logique pour indiquer que l'utilisateur a arrêté de taper
    ...
    /**
     * Sécurisation des entrées utilisateur (améliorée)
     */
    sanitizeInput(input) {
        if (!input) return '';
    ...
    validateInput(input) {
        ...
        if (input.length > 5000) return false; // Limite augmentée
    ...
    /**
     * Changement de personnalité avec animation
     */
    ...
    confirmClearChat() {
        if (this.conversationHistory.length === 0) {
            this.showSecureMessage('ai', 'La conversation est déjà vide.');
            return;
        }
    ...
    toggleVoiceRecording() {
        if (!this.recognition) {
            this.showSecureMessage('ai', 'Reconnaissance vocale non supportée sur ce navigateur.');
            return;
        }
    ...
    /**
     * Messages de personnalité
     */
    getPersonalityMessage() {
        const messages = {
            casual: { 
                fr: "Cool ! Je suis maintenant en mode décontracté 😎", 
                en: "Cool! I'm now in casual mode 😎" 
            },
            friendly: { 
                fr: "Parfait ! Je suis maintenant super amical ! 😊", 
                en: "Perfect! I'm now super friendly! 😊" 
            },
            professional: { 
                fr: "Très bien. Je suis maintenant en mode professionnel.", 
                en: "Very well. I am now in professional mode." 
            },
            creative: { 
                fr: "Fantastique ! Mon imagination s'éveille... ✨", 
                en: "Fantastic! My imagination awakens... ✨" 
            },
            funny: { 
                fr: "Haha ! Mode blague activé ! 😂", 
                en: "Haha! Joke mode activated! 😂" 
            }
        };
        
        return messages[this.personality]?.[this.language] || messages[this.personality]?.fr || "Prêt !";
    }
    ...
    getDefaultResponse() {
        const responses = {
            casual: { 
                fr: "Ah ouais, je vois ce que tu veux dire ! 😄", 
                en: "Oh yeah, I see what you mean! 😄" 
            },
            friendly: { 
                fr: "C'est une excellente question ! 😊", 
            ...
            creative: { 
                fr: "Quelle idée intéressante ! ✨", 
                en: "What an interesting idea! ✨" 
            },
            funny: { 
                fr: "Haha, bonne question ! 😂", 
                en: "Haha, good question! 😂" 
            }
        };
    ...
    /**
     * Affichage sécurisé des messages avec animations
     */
    ...
    /**
     * Réception d'image depuis Android
     */
    receiveImageFromAndroid(imageBase64, fileName) {
        this.showSecureMessage('user', `📷 Image partagée: ${this.sanitizeInput(fileName)}`);
        ...
        caption.textContent = `📷 ${this.sanitizeInput(fileName)}`;
    ...
    receiveFileFromAndroid(fileName, fileContent, fileType) {
        ...
        if (fileType && (fileType.startsWith('text/') || fileType === 'application/json')) {
            ...
            this.showSecureMessage('ai', `📄 Contenu de ${safeFileName}:\n\n${preview}`);
        }
    }
    ...
    switchView(viewId) {
        ...
        if (viewId === 'view-config') {
            ...
            // Initialiser l'accordéon après un petit délai pour laisser le DOM se stabiliser
            setTimeout(() => {
                this.initConfigTabs();
    ...
    /**
     * Transforme chaque .panel-section en accordéon (toggle sur le titre)
     */
    initConfigTabs() {
    ...
    /**
     * Chargement de l'aperçu de la configuration IA
     */
    ...
    /**
     * Ouverture de l'éditeur de configuration IA
     */
    ...
    async reloadAiConfigEditor() {
        ...
        if (this.aiConfigFeedback) {
            this.aiConfigFeedback.textContent = 'Rechargé depuis le stockage.';
            this.aiConfigFeedback.style.color = '#94a3b8';
        }
    }
    ...
    /**
     * Fermeture de l'éditeur de configuration
     */
    closeAiConfigEditor() {
        if (this.jsonModal) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('✅ Application Chat IA initialisée avec succès');
});